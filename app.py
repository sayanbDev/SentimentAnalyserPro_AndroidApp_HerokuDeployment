...
def home():
    ...
    api = tweepy.API(auth)
    tweets = api.search('BJP -filter:retweets', result_type="mixed", lang="en",tweet_mode="extended",count=100)
...
def get_text_prediction():
    ...
    if no_of_tweets<0 or no_of_tweets>100:
        ...
        if len(tweets)==no_of_tweets:
            ...
            for tweet in tweets:
                try:
                    data.append(clean_tweet(tweet.retweeted_status.full_text))
                except:
                    data.append(clean_tweet(tweet.full_text))
            ...
            for tweet in data:
                ...
                if analysis.sentiment.polarity > 0: 
                    ...
                    if hasattr(tweets[i],'retweeted_status'):
                        count = tweets[i].retweeted_status.favorite_count
                        pos_text.append(tweets[i].retweeted_status.full_text)
                    else:
                        count = tweets[i].favorite_count
                        pos_text.append(tweets[i].full_text)
                        
                    pos_likes.append(str(count))
                    ...
                    if hasattr(tweets[i],'retweeted_status'):
                        count = tweets[i].retweeted_status.favorite_count
                        neu_text.append(tweets[i].retweeted_status.full_text)
                    else:
                        count = tweets[i].favorite_count
                        neu_text.append(tweets[i].full_text)
                    neu_likes.append(str(count))
                else: 
                    ...
                    if hasattr(tweets[i],'retweeted_status'):
                        count = tweets[i].retweeted_status.favorite_count
                        neg_text.append(tweets[i].retweeted_status.full_text)
                    else:
                        count = tweets[i].favorite_count
                        neg_text.append(tweets[i].full_text)
                    neg_likes.append(str(count))
                i+=1
            overall=''
